...
export const mapUpdatedArticleFields = (existing: Article, updated: ArticleDetailDto) => {
...
export const toArticleDetailDto = (article: Article): ArticleDetailDto => {
    ...
    return { content: article.articleContent.content, comments: article.comments.map(toCommentDto), ...articleDto };
};
